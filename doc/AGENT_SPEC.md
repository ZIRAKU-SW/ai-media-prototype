...
### ⚠️ X ログイン失敗時の鉄則（台帳 #28）

**ログイン（ブラウザ自動化 `x:browser` 系）に失敗したら、1回でやめて報告し、ユーザーの指示を仰ぐこと。**

- 連続ログイン試行は X のレート制限（アカウント一時凍結）を招く。デバッグ目的でも再実行を繰り返さない。
- `browser-post.ts` は失敗時に `data/x-login-lock.json` を書き、解除（`npm run x:browser -- --clear-login-lock`）するまで再実行を自動拒否する。**勝手にロックを解除して再試行しない。**
- 原因調査は本番ログインを叩かず、`--dry-run` か単発の DOM 観察に留める。
- 本番投稿はブラウザ自動化ではなく **API 方式（`npm run x:post`）を優先**（UI 変更・レート制限に強い）。
...
| 日付 | 区分 | 内容 |
|------|------|------|
| 2026-06-13 | feat | 記事公開→自動X投稿を **VM cron 方式**で実装（Zapier公式Xアクション廃止のため方針転換）。`scripts/x/post-articles.ts`＝既存 `post-tweet.ts`(twitter-api-v2 OAuth署名)再利用、Supabase REST直叩きで会員限定除外、`data/x-article-history.json`(slug)で重複防止、`x:articles:seed` でバースト防止(30件seed済)、X重み付き文字数で280以内トランケート、1回最大3件・失敗即停止。稼働には X_ACCESS_TOKEN/SECRET 追加＋cron有効化が必要。`npm run x:articles{,:dry,:seed}` |
